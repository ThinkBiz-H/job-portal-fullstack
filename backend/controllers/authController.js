...
/* ================= UPDATE EMPLOYER PROFILE (🔥 FINAL FIX) ================= */
exports.updateEmployerProfile = async (req, res) => {
  try {
    const user = await User.findById(req.user.id);
    ...
    // 🔥 MAIN FIELDS
    user.employerProfile.gstNumber = profile.gstNumber || "";
    user.employerProfile.cinNumber = profile.cinNumber || "";
    user.employerProfile.verificationStatus = "pending";

    // 🔥 MERGE ALL FIELDS
    Object.keys(profile).forEach((key) => {
      user.employerProfile[key] = profile[key];
    });
